...
TrackInformation ::TrackInformation(const TrackInformation* aTrackInfo)
  : G4VUserTrackInformation()
{
  fIsAllowedInelastic = aTrackInfo->fIsAllowedInelastic;
  fMotherPDGid = aTrackInfo->fMotherPDGid;
  fTrackType = aTrackInfo->fTrackType;
  fHasClone = aTrackInfo->fHasClone;
}
...
{
  G4cout 
    << "Track is allowed to interact inelastically: " << fIsAllowedInelastic << G4endl
    << "    Mother PDGid = " << fMotherPDGid << G4endl
    << "    Track type   = " << fTrackType << G4endl;
}
